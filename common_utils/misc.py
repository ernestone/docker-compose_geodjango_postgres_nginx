...
def zip_files(zip_path, file_paths, base_path=None):
    """
...
def get_country_info(name_country, lang='en'):
    ...
    trans = None
    if lang.lower() != 'en':
        trans = gettext.translation('iso3166', pycountry.LOCALES_DIR, languages=[lang])
...
def tree_subdirs(path_dir_base, relative_dirs_sel=None, last_level_as_list=False):
    ...
    if relative_dirs_sel:
        for dir_sel in relative_dirs_sel:
            if os.path.exists(path_dir_rel := os.path.join(path_dir_base, dir_sel)):
...
def tree_paths(path_dir_base, relative_dirs_sel=None, func_filter_path=None, solo_dirs=False):
    ...
    for dir_path, dir_names, file_names in os.walk(path_dir_base):
        ...
        files_selected = {fn: None for fn in file_names if not func_filter_path or func_filter_path(fn)}
...
def random_brewer_colour(map_type='diverging', name='RdYlBu', number=11):
    """
    ...
    Returns:
        hex (str), name (str)
    """
    id_col = random.randint(1, number)

    return brewer_colour(id_col, map_type, name, number)
